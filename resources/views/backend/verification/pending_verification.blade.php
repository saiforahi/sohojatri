                    ...
                    <div class="row">
                        ...
                        <div class="col-6 my-2" style="width: 60%; margin: 15px auto; box-shadow: 0px 0px 5px 5px #9e9c9c; padding: 0px;border-radius: 5px 0 0 5px;">
                            ...
                            <div class="card-header bg-success text-white cardHead">
                                Current Verification Request:  &nbsp;(  @if($type=='n') NID
                                @elseif($type=='p') Passport
                                @elseif($type=='d')Driving
                                @endif )
                            </div>
    ...
    margin-top: 7px;" data-toggle="modal" data-target="#imgmodal1"><i class="fas fa-plus"></i></button><br>
                    ...
                    <form method="post" class="row" action="{{url('admin-pending-verification-change'.'?a=nid&b=del&c='.$verifications->id)}}">
                        ...
                        <div class="col-6 my-2">
                            ...
                            <a type="" class="btn btn-sm btn-primary" href="{{ route('admin.pending.verification') }}">Pending</a>
        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                <table class="table border" id="example">
                    ...
                    <tbody>
                    <?php $listNum = 1; ?>
                    @foreach($result_arr as $verification)
                        <tr>
                            <td>{{$listNum}}</td>
                            ...
                            <td>
                                <a href="{{route('admin.pending.verification').'/'.$verification->id.'/'.$verification->type}}"
      ...
      <div class="modal-body" style=" padding-left: 103px;">
        ...
        <form method="post" class="row" action="{{url('admin-pending-verification-change'.'?a=nid&b=del&c='.$verifications->id)}}">
                        ...
                        <select name="cbo_rej_mess" id='cbo_rej_mess' class="messreject" style="padding-left: 61px;">
                                ...
                                @foreach ($rejectmessage as $reject)
                                     <option value="{{$reject->reject_message}}">{{$reject->reject_message}}</option>
                                @endforeach
...
<div class="modal fade" id="addreject" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    padding-right: 124px;">
       <form class="row" id="addform" method="post" action="{{url('/add')}}">
 
    
        {{ csrf_field() }}
  
   
    <input type="text" name="reject_message" class="form-control" id="reject_message" placeholder="Reject Reason">
    ...
    width: 321%;">
     @if($type=='n')
       <img src="{{asset('/'.$verifications->nid_image1)}}" class="img-thumbnail w-25 imgshowmodal" style="">
        @elseif($type=='p')
        <img src="{{asset('/'.$verifications->passport_image1)}}" class="img-thumbnail w-25 imgshowmodal" >
         @elseif($type=='d')
           <img src="{{asset('/'.$verifications->driving_image1)}}" class="img-thumbnail w-25 imgshowmodal" >
           @endif
      </div>
      @endif
      <div class="modal-footer">
    ...
    width: 321%;">
      @if($type=='n')
       <img src="{{asset('/'.$verifications->nid_image2)}}" class="img-thumbnail w-25 imgshowmodal" style="">
          @elseif($type=='p')
        <img src="{{asset('/'.$verifications->passport_image2)}}" class="img-thumbnail w-25 imgshowmodal" >
         @elseif($type=='d')
           <img src="{{asset('/'.$verifications->driving_image2)}}" class="img-thumbnail w-25 imgshowmodal" >
           @endif
      </div>
      @endif
      <div class="modal-footer">
